...
class TransformerAdapter(nn.Module):
    def __init__(self, opt, dictionary):
        ...
        self.embeddings = nn.Embedding(len(dictionary), opt.embeddings_size, padding_idx=self.pad_idx) # Embedding Layer
        if not opt.learn_embeddings:
            self.embeddings.weight.requires_grad = False
        nn.init.normal_(self.embeddings.weight, mean=0, std=0.05) # Initialize weights of embedding layer
        dropout = opt.transformer_dropout if opt.transformer_dropout else 0 # Dropout Layer
