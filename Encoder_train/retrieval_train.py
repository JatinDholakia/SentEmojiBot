...
def train(epoch, start_time, model, optimizer, opt_, data_loader):
    ...
    for idx, ex in enumerate(data_loader, 1):
        ...
        loss, ok = loss_fn(*model(*params))  # forward pass
        nb_ok += ok
        nb_exs += ex[0].size(0)
...
def validate(env,
    ...
    for i, ex in enumerate(data_loader):
        ...
        # print(params[0].size())
        # print(params[1].size())
        # print(env.to_words(params[0][0]))
        print(env.to_words(params[0][0]))
        print(env.to_words(params[1][0]))
        # print(env.to_words(params[1][0]))
        ...
        if examples >= max_exs and dtype == "reddit":
            break
    # print("sum_losses = " + str(sum_losses))
    # print("correct = " + str(correct))
    # print("n_losses = " + str(n_losses))
    n_examples = 0
    if len(all_context) > 0: # Evaluation metrics and avg loss
...
def main(opt_):
    if opt_.pretrained: # Used for evaluation
        ...
        train_model(opt_) #Used for training
